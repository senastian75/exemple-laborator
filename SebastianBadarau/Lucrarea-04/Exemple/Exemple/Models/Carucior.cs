...
namespace Exemple.Domain.Models
{
    ...
    public static partial class Carucior
    {
        ...
        {
            public ValidatedCarucior(IReadOnlyCollection<ValidatedProduct> productlist)
            {
                ProductList = productlist;
            }
            public IReadOnlyCollection<ValidatedProduct> ProductList { get; }
        }
        public record FailedCarucior : ICarucior
        {
            internal FailedCarucior(IReadOnlyCollection<UnvalidatedProductQuantity> productList, Exception exception)
            {
                ProductList = productList;
                Exception = exception;
            }
        ...
        {
            internal CalculatePrice(IEnumerable<CalculatedPrice> productlist)
            {
                ProductList = productlist;
            }
            public IEnumerable<CalculatedPrice> ProductList { get; }
        }
        public record PublishedCarucior : ICarucior
        {
            internal PublishedCarucior(IReadOnlyCollection<CalculatedPrice> productList, DateTime publishedDate, string csv)
            {
                ProductList = productList;
